  ...
  {#snippet children([models, providers, keys], reload)}
    ...
    <div class="space-y-6">
      <Card>
        ...
        {#if notice}
          <p class="mt-2 text-sm text-green-500">{notice}</p>
        {/if}
        {#if error}
      ...
      <Card>
        <h2 class="mb-3 text-sm font-semibold">Add a custom model</h2>
        ...
        <table class="w-full text-left text-sm">
          <thead class="bg-surface text-muted">
            <tr>
              ...
              <th class="px-4 py-2 font-medium">Cost (in/out)</th>
          ...
          <tbody>
            {#each filteredModels as model (model.userModelId)}
              <tr class="border-t border-border">
                <td class="px-4 py-2">
                  ...
                  <span class="ml-2 text-xs text-muted">{model.modelId}</span>
                  {#if model.isFree}
                ...
                <td class="px-4 py-2 text-muted">{model.speedTier}</td>
                <td class="px-4 py-2">
                  <Toggle
            ...
            {:else}
              <tr>
                <td class="px-4 py-6 text-center text-muted" colspan="6">
