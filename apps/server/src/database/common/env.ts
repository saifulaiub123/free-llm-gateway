 ...
 * keeps the provider modules and common code free of scattered `process.env` reads.
 */

/** Raw `DB_PROVIDER` (which provider to use); `undefined` lets the registry apply its default. */
export const dbProvider = (): string | undefined => process.env.DB_PROVIDER;

/** Table-name prefix prepended to every table by the active provider's creator. */
